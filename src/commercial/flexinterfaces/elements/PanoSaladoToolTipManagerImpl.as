 ...
 */
public class PanoSaladoToolTipManagerImpl extends EventDispatcher
             implements IPanoSaladoToolTipManager
{
     ...
     *  @private
     */
    private static var instance:IPanoSaladoToolTipManager;
     ...
     *  @private
     */
    public static function getInstance():IPanoSaladoToolTipManager
    {
        if (!instance)
            instance = new PanoSaladoToolTipManagerImpl();
     ...
     *  @private
     */
    public function PanoSaladoToolTipManagerImpl()
    {
        super();
     ...
     */
    protected var previousTarget:DisplayObject;
     ...
     *  @private
     */
    private var _toolTipClass:Class = HTMLToolTip;
    ...
    private var registeredDisplayObjects : Dictionary = new Dictionary( true );
     ...
     *  over the target.
     */
    public function registerMouseToolTip(target:DisplayObject,
                                    oldToolTip:String,
									newToolTip:String):void
    {
        if (!oldToolTip && newToolTip)
        {
			...
			// code to add object to registeredDisplayObjects Dict
			registeredDisplayObjects[ target ] = "mouse";
        ...
        else if (oldToolTip && !newToolTip)
        {
			...
			registeredDisplayObjects[ target ] = null;
...
//  public function registerErrorString(target:DisplayObject,
	public function registerDisplayObjectToolTip(target:DisplayObject,
										...
										newErrorString:String):void
    {
        if (!oldErrorString && newErrorString)
        {
			...
			registeredDisplayObjects[ target ] = "displayObject";
    ...
    {
    	var oldShowDelay:Number = PanoSaladoToolTipManager.showDelay;
    	PanoSaladoToolTipManager.showDelay = 0;
    	checkIfTargetChanged(target);
    	PanoSaladoToolTipManager.showDelay = oldShowDelay;
    }
     ...
     */
    protected function checkIfTargetChanged(displayObject:DisplayObject):void
    {
        if (!enabled)
            return;
     ...
     */
    protected function findTarget(displayObject:DisplayObject):void
    {
        ...
        while (displayObject)
        {
            if (registeredDisplayObjects[ displayObject ] == "displayObject")
            {
                if ( displayObject.hasOwnProperty("panoSaladoToolTip") )
					currentText = displayObject[ "panoSaladoToolTip" ] as String;
            ...
            if (registeredDisplayObjects[ displayObject ] == "mouse")
            {
                 if ( displayObject.hasOwnProperty("panoSaladoToolTip") )
					currentText = displayObject[ "panoSaladoToolTip" ] as String;
                if (currentText != null)
     ...
     */
    protected function targetChanged():void
    {
     ...
     */
    protected function initializeTip():void
    {
        // Set the text of the tooltip.
        if (currentToolTip is HTMLToolTip)
            HTMLToolTip(currentToolTip).text = currentText;

         if (!isTail)
		{
			if ( currentToolTip is IStyleClient )
				IStyleClient(currentToolTip).setStyle("borderStyle", "taillessTip");
		}
    ...
    protected function positionTip():void
    {
        ...
        if (isTail)
        {
            ...
            var inTop:Boolean = false;
            var inRight:Boolean = false;
            var inBottom:Boolean = false;

            var targetGlobalBounds:Rectangle = getGlobalBounds(currentTarget);
            
            //exceeds top, assumes tooltip above extending right from target upper left corner
            if (targetGlobalBounds.top - currentToolTip.height - 11 > 0)
            	inTop = true;
            //exceeds right, assumes tooltip above extending right from target upper left corner
            if (targetGlobalBounds.left + currentToolTip.width < screenWidth)
            	inRight = true;
            //exceeds bottom, assumes tooltip below extending right from target lower left corner
            if (targetGlobalBounds.bottom + currentToolTip.height + 11 < screenHeight)
            	inBottom = true;
            	
            
            //default is above (tip extending right from above the upper left of target)
			if ( inTop && inRight )
			{ 
				//tailTipAbove
				if (currentToolTip is IStyleClient)
					IStyleClient(currentToolTip).setStyle("borderStyle", "tailTipAbove");
				x = targetGlobalBounds.left - 2;
				y = targetGlobalBounds.top - currentToolTip.height + 1;
            }
            ...
            { 
            	//tailTipBelow
            	if (currentToolTip is IStyleClient)
					IStyleClient(currentToolTip).setStyle("borderStyle", "tailTipBelow");
				x = targetGlobalBounds.left - 2;
				y = targetGlobalBounds.bottom - 1;
            }
            ...
            { 
            	//tailTipAboveLeft
            	if (currentToolTip is IStyleClient)
					IStyleClient(currentToolTip).setStyle("borderStyle", "tailTipAboveLeft");
				x = targetGlobalBounds.left - currentToolTip.width - 1;
				y = targetGlobalBounds.top - currentToolTip.height + 1;
            }
            ...
            { 
            	//tailTipBelowLeft
            	if (currentToolTip is IStyleClient)
					IStyleClient(currentToolTip).setStyle("borderStyle", "tailTipBelowLeft");
				x = targetGlobalBounds.left - currentToolTip.width -1;
				y = targetGlobalBounds.bottom + 11;
            }
...
//        }
    ...
    public function createToolTip(text:String, x:Number, y:Number,
                                         tailTipBorderStyle:String = null,
                                         context:IUIComponent = null):IToolTip
    {
        var toolTip:ToolTip = new toolTipClass();
        ...
        if (tailTipBorderStyle)
        {
            toolTip.setStyle("styleName", "tailTip");
            toolTip.setStyle("borderStyle", tailTipBorderStyle);
        }
     ...
     *  @private
     */
    protected function showEffectEnded():void
    {
        if (hideDelay == 0)
        {
            hideTip();
     ...
     *  @private
     */
    protected function hideEffectEnded():void
    {
        reset();
     ...
     *  with an errorString.
     */
    protected function errorTipMouseOverHandler(event:MouseEvent):void
    {
        checkIfTargetChanged(DisplayObject(event.target));
    }
     ...
     *  with an errorString.
     */
    protected function errorTipMouseOutHandler(event:MouseEvent):void
    {
        checkIfTargetChanged(event.relatedObject);
    }
     ...
     */
    protected function showTimer_timerHandler(event:TimerEvent):void
    {
